...
export const addNpmPackageName = async () => {
    ...
    const { npm = [], unknown = [] } = Object.groupBy(packages, (p) => ("npmPkg" in p ? "npm" : "unknown"));

    logger.log(`package without known npm package [${unknown.length}/${unknown.length + npm.length}]`);
