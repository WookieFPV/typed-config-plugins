...
const logger = stepLogger("Detect Config Plugins");
...
export const detectConfigPlugins = async (type: "onlyNew" | "full" = "onlyNew"): Promise<Array<RnDepFull>> => {
    ...
    const packagesToCheck = allPackages.filter((p) => "npmPkg" in p && (type === "full" || !("hasConfigPlugin" in p)));
    logger.log(`Checking ${packagesToCheck.length} packages`);
